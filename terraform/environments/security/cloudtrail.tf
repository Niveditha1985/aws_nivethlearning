...
# ============================================================================
# This file configures centralized CloudTrail logging in the Security account.
# The organization trail is created in the Management account and writes to
# this centralized S3 bucket.
#
# Architecture:
# - Management Account: Creates organization-wide CloudTrail trail
# - Security Account: Provides S3 bucket and KMS key for centralized log storage
#
...
# Centralized CloudTrail logs bucket (receives logs from all organization accounts)
...
resource "aws_s3_bucket_policy" "cloudtrail_logs" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        Sid    = "AWSCloudTrailWriteForOrganization"
        Effect = "Allow"
        Principal = {
          Service = "cloudtrail.amazonaws.com"
        }
        ...
        Condition = {
          StringEquals = {
            "s3:x-amz-acl" = "bucket-owner-full-control"
            "aws:SourceArn" = "arn:aws:cloudtrail:${var.aws_region}:${var.management_account_id}:trail/*"
          }
        }
      },
      {
